...
class Spline{
    ...
    void redistribute(){
        ...
        for(int i=0;i<size;i++,dt+=ddt,t+=dt){
            time[i]=t;
        }
        time[size-1]=1;
    }
    ...
    bool pick(const Vector& lb, const Vector& rt){
        ...
        for(int i=0;i<res;i++){
            ...
            if(canDraw(t)){
                ...
                if(i>1 && cohenSutherland(prevCopy, currentCopy, lb, rt)){
                    return true;
                }
                prev=current;
            }
    ...
    bool cohenSutherland(Vector& p0, Vector& p1, const Vector& lb, const Vector& rt){
        unsigned char c0=code(p0, lb, rt);
        unsigned char c1=code(p1, lb, rt);
        Vector p;
        while(1){
            if(!(c0|c1)){
                return true;
    ...
    unsigned char code(Vector& p, const Vector& lb, const Vector& rt){
        return (p.x<lb.x)<<3|(p.x>rt.x)<<2|(p.y<lb.y)<<1|(p.y>rt.y);
    }
...
class CRSpline: public Spline{
    ...
    bool canDraw(double t){
        return getIndex(t)>0 && getIndex(t)<size-2;
    }
};
class KKSpline: public Spline{
    ...
    Vector r(double t){
        ...
        if(i==0){
            return lagrange(t, &(points[i]), 3, &(ti[i]));
        }
        else if(i>size-3){
            return lagrange(t, &(points[size-3]), 3, &(ti[size-3]));
        }
