...
Actor hansel(Color(0.16, 0.556, 1.0), true);
Actor gretel(Color(1.0, 0.67, 1.0),false);
Tower tower;
void Actor::draw()
{
    float R=0.05;
    ...
    for(float i=0;i<3.1415*2;i+=3.1415*2/100)
    {
        glVertex2f(r.x+xOffset+R*sin(i), r.y+yOffset+R*cos(i));
    }
    glEnd();
...
void Actor::redirect(float nx, float ny)
{
    float dx = r.x - nx;
    float dy = r.y - ny;
    float vx = -sin(atan2(dx, dy)) * vmax;
    float vy = -cos(atan2(dx, dy)) * vmax;
    d.x = vx;
    d.y = vy;
}
void Actor::tick()
...
void Tower::draw()
{
    ...
    glVertex2f(r.x - 0.05, r.y - 0.05);
    glVertex2f(r.x, r.y + 0.05);
    glVertex2f(r.x + 0.05, r.y - 0.05);
    glEnd();
}
...
float gradf(float x, float y, Vector v)
{
    ...
    v = v * (1 / v.Length());
    Vector gf = Vector(
                    2292 * cos(6 * x) * cos(10 * x * y) - 3820 * y * sin(6 * x) * sin(10 * x * y),
                    -3820 * x * sin(6 * x) * sin(10 * x * y));
    return gf * v;
}
float col(float h)
{
...
void onDisplay( )
{
    ...
    if (hansel.canTransmit(tower) && gretel.canTransmit(tower)) {
        ...
        glBegin(GL_TRIANGLES);
        glVertex2f(hansel.r.x, hansel.r.y);
        glVertex2f(gretel.r.x, gretel.r.y);
        glVertex2f(tower.r.x, tower.r.y);
        glEnd();
    }
    glColor3f(.0, .0, .0);
    glBegin(GL_POLYGON);
    ...
    glBegin(GL_POLYGON);
    glVertex2f(-.5, .95);
    glVertex2f((float)frameCanTransmit / frame - 0.5, .95);
    glVertex2f((float)frameCanTransmit / frame - 0.5, .85);
    glVertex2f(-.5, .85);
    glEnd();
    glutSwapBuffers();
}
...
void onIdle( )
{
    ...
    if ((time - lastSim) / dt) {
        for (t = 0; t < (time - lastSim) / dt; t++) {
            ...
            hansel.tick();
            gretel.tick();
            if (gretel.canTransmit(tower) && hansel.canTransmit(tower)) {
                frameCanTransmit++;
            }
        }
        lastSim += t * dt;
